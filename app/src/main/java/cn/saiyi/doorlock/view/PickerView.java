...
public class PickerView extends View
{
	...
	private void drawData(Canvas canvas)
	{
		...
		float size = (mMaxTextSize - mMinTextSize-10) * scale;
		float x1 = (float) (mViewWidth / 2.0);
		float y1 = (float) (mViewHeight / 2.0);
		Log.e("number",y1+"");
		canvas.drawLine(0,y1-y1/3+y1/12,getWidth(),y1-y1/3+y1/12,mPaint);
		canvas.drawLine(0,y1+y1/3-y1/12,getWidth(),y1+y1/3-y1/12,mPaint);
		mPaint.setColor(mColorText);
		...
		tx = mDataList.get(mCurrentSelected);
		canvas.drawText(mDataList.get(mCurrentSelected), x, baseline, mPaint);
		mPaint.setColor(0xE7E7E7);
		// 绘制上方data
